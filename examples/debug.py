import asyncio
import time
from onesmartcontrolpy.const import *
from onesmartcontrolpy.socket import OneSmartSocket
import logging
...
async def setup(host, port=9010, username="admin", password=None):
	await gateway.connect(host, port)
	await gateway.authenticate(username, password)
...
async def run():
	host = input("OneConnect hostname or IP address: ")
	username = input("Username: ")
	password = input("Password: ")
	await setup(host=host, username=username, password=password)
	...
	for device in devices:
		...
		async def fetch_attribute_values(device, device_id, device_attributes):
			...
			try:
				print(f"Fetching attribute values for { device_id }: { device_attributes }")
